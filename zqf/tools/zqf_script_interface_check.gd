...
# add #@implements ifoo
# at the START of a gdscript file to implement the interface 'ifoo' in a script
# Run from gdscript editor
# file -> run or ctrl + shift + x
# view results in output window.
# can implement multiple interfaces, eg:
...
func _find_all_files_with_extension(
	path:String, dir:Directory, results:Dictionary, extension:String) -> void:
	var err = dir.open(path)
	if err != OK:
		print("Failed to open dir " + path + " code " + str(err))
		return
	var dirPaths = []
	dir.list_dir_begin()
	...
	while fileName != "":
		if dir.current_is_dir():
			dirPaths.push_back(path + "/" + fileName)
		else:
	...
	for dirPath in dirPaths:
		_find_all_files_with_extension(dirPath, dir, results, extension)
...
func _check_implementation(
	...
	for key in interfaceFuncKeys:
		if !implemented.has(key):
			print(">>> missing function " + str(functions[key]))
			continue
		# compare signatures
...
func _check_for_interfaces(gdScriptPath:String, interfaces:Dictionary) -> void:
	var f:File = File.new()
	var err = f.open(gdScriptPath, File.READ)
	if err != OK:
		print("Failed to open " + gdScriptPath + " code " + str(err))
		return
...
func _full_project_scan() -> void:
	...
	_find_all_files_with_extension("res://", dir, files, ".gd")
